...
try:
    ...
    content_length = 0
post_body = sys.stdin.read(content_length) if content_length else ""
form = dict(parse_qsl(post_body, keep_blank_values=True))

project = form.get("project", "").strip()
ftype   = form.get("type", "").strip()
